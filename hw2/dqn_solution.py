"""
Deep Q-Network (DQN) for Homework 2  —  v2  (Apple Silicon MPS optimised)
==========================================================================
Key improvements over v1:
  - MPS (Metal Performance Shaders) support for Apple M1/M2/M3 chips
  - 2000 training episodes instead of 500
  - Larger replay buffer (50k) for better experience diversity
  - Huber loss instead of MSE (more robust to outliers)
  - Learning rate scheduler (StepLR)
  - Faster epsilon decay so agent exploits sooner
  - Larger batch size (64) to make better use of MPS parallelism
  - DataLoader num_workers=0 (required on macOS)
...
# ─────────────────────────────────────────────
# 2.  HYPER-PARAMETERS  (tuned for faster convergence)
# ─────────────────────────────────────────────
N_ACTIONS          = 8
GAMMA              = 0.99
EPSILON_START      = 1.0
EPSILON_DECAY      = 0.997        # faster than 0.999
EPSILON_DECAY_ITER = 10
MIN_EPSILON        = 0.05         # lower floor → more exploitation
LEARNING_RATE      = 3e-4         # slightly higher than 1e-4
BATCH_SIZE         = 64           # larger batch → better MPS utilisation
UPDATE_FREQ        = 4
TARGET_UPDATE_FREQ = 200          # sync target less often → more stable
BUFFER_LENGTH      = 50_000       # bigger buffer → more diverse samples
N_EPISODES         = 2000         # 4x more training
WARMUP_STEPS       = 1000         # more random exploration before learning
LR_STEP            = 500          # decay LR every N episodes
...
class DQNAgent:
    def __init__(self, state_dim, n_actions):
        ...
        # Decay LR by 0.5 every LR_STEP episodes worth of updates
        self.scheduler = optim.lr_scheduler.StepLR(
            self.optimizer,
            step_size=LR_STEP * 50 // UPDATE_FREQ,  # approx steps per LR_STEP episodes
            gamma=LR_GAMMA
        )
    ...
    def update(self):
        ...
        # Huber loss (smoother than MSE for large errors)
...
if __name__ == "__main__":
    ...
    print(f"        Device={DEVICE} | Episodes={N_EPISODES} | Buffer={BUFFER_LENGTH} | Batch={BATCH_SIZE}")
    print("=" * 60)
